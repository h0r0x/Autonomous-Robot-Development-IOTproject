...
def region_of_interest(edges):
    ...
    polygon = np.array([[
        (0, height * 1 / 2 + off_set_region),
        (width, height * 1 / 2 + off_set_region),
        (width, height),
        (0, height),
    ]], np.int32)
...
while True:
    ...
    if num_lanes == 2:
        ...
        x_offset = ((left_x2 + right_x2) / 2 - mid)*x_adjust_2lanes
        y_offset = 180
        ...
        #steering_angle = stabilize_steering_angle(steering_angle,angle_in_deg+90,2)
        steering_angle = angle_in_deg+90
        # valore da passare al pi
        angle_value_pi = steering_angle - 90
        
        if angle_value_pi < 0:
            angle_value_pi = max(angle_value_pi,-10)
        else:
            angle_value_pi = min(angle_value_pi,10)
            
        steering_angle = angle_value_pi+90
    
        print(angle_value_pi)
        ...
        '''
        if angle_value_pi < 0:
            angle_value_pi = max(angle_value_pi,-15)
        else:
            angle_value_pi = min(angle_value_pi,15)
        steering_angle = angle_value_pi +90
        
        '''
        if n > 20:
            px.set_dir_servo_angle(angle_value_pi)
            px.forward(0.1)
            #time.sleep(0.075)
            #px.forward(0)
